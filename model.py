...
class FakeNewsDetector:
    ...
    def __init__(self, dataset_path: Any, artifact_dir: Any) -> None:
        self.artifact_dir = self._coerce_path(artifact_dir, fallback=Path.cwd() / "artifacts")
        self.dataset_path = self._coerce_path(
            dataset_path,
            fallback=self.artifact_dir.parent / "data" / "WELFake_Dataset.csv",
            base_dir=self.artifact_dir.parent,
        )
        self.model_path = self.artifact_dir / "fake_news_pipeline.joblib"
        self.metrics_path = self.artifact_dir / "metrics.json"
        self.history_path = self.artifact_dir / "history.db"
    ...
    def _coerce_path(
        ...
        candidate = Path(text).expanduser() if text else fallback
