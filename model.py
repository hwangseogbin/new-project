...
class FakeNewsDetector:
    ...
    def _load_or_train(self) -> TrainingArtifacts:
        ...
        if self.model_path.exists() and self.metrics_path.exists():
            ...
            if (
                metrics.get("model_version") == self.MODEL_VERSION
                and self._training_signatures_match(
                    metrics.get("training_signature"),
                    current_training_signature,
                )
                and isinstance(payload, dict)
                and {"headline", "body", "fusion"} <= payload.keys()
            ):
                metrics["training_signature"] = current_training_signature
